...
#
#   scripts/compute_tag.sh <version-name> <is_beta>
#
# Stable releases are tagged v<version-name>. Betas are tagged
# v<version-name>-beta.<n>, where <n> is one past the highest beta already
# tagged for that version name, so a series looks like:
#
#   v2.5.1            stable
#   v2.6.0-beta.1
#   v2.6.0-beta.2
...
#
# The build number stays out of the tag; it lives in pubspec.yaml only, where
# Play and App Store Connect need it.
#
# Needs the full tag list, so fetch with fetch-depth: 0 under Actions. Prints
# the tag and, under Actions, writes tag and beta_number to $GITHUB_OUTPUT.
